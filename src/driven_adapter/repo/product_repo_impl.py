...
class ProductRepoImpl(IProductRepo):
    ...
    @staticmethod
    def _to_user_entity(db_user: UserModel) -> User:
        return User(
            id=db_user.id,
            email=db_user.email,
            name=db_user.name,
    ...
    async def get_by_id_with_seller(
        ...
        result = await self.session.execute(
            select(ProductModel, UserModel)
            .join(UserModel, ProductModel.seller_id == UserModel.id)
        ...
        db_product, db_user = row
        product = ProductRepoImpl._to_entity(db_product)

        return product, ProductRepoImpl._to_user_entity(db_user)
